      ...
      >
        {thumbnailItems.length > 0 && (
          <div
            ...
            className="bg-bkg-low grid grid-cols-[repeat(auto-fit,_minmax(169px,_169px))] justify-center justify-items-center gap-[6px]"
          >
        ...
        {listItems.length > 0 && (
          <div
            id="ohif-thumbnail-list"
            className="bg-bkg-low grid grid-cols-1 place-items-start gap-[4px]"
          >
            {listItems.map(item => {
              const { displaySetInstanceUID, componentType, numInstances, ...rest } = item;
              const isActive = activeDisplaySetInstanceUIDs.includes(displaySetInstanceUID);
